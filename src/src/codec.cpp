...
{
    std::size_t serialize(xalt::tlist<std::string> tag, void* output, const std::string& message)
    {
        std::copy(message.begin(), message.end(), static_cast<std::uint8_t*>(output));
        return size(tag, message);
    }

    std::string deserialize(xalt::tlist<std::string> /* t */, const void* input, std::uint64_t size)
    {
...
#define NIL_SERVICE_CODEC_DEFINE(TYPE)                                                             \
    std::size_t serialize(xalt::tlist<TYPE> tag, void* output, TYPE data)                          \
    {                                                                                              \
